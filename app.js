...
function renderCalendarDays(days, showWeekday) {
  ...
  days.forEach((date) => {
    ...
    dayTasks.forEach((task) => {
      ...
      item.title = "タスクカードへ移動して編集";
      item.addEventListener("click", () => focusTaskAndEdit(task.id));
...
function focusTaskAndEdit(id) {
  const card = document.querySelector(`[data-task-id="${cssEscape(id)}"]`);
  if (card) {
    card.scrollIntoView({ behavior: "smooth", block: "center" });
    card.classList.add("is-linked");
    window.setTimeout(() => card.classList.remove("is-linked"), 1600);
  }
  window.setTimeout(() => openEditDialog(id), card ? 260 : 0);
}
...
function registerServiceWorker() {
  ...
  window.addEventListener("load", async () => {
    try {
      const registration = await navigator.serviceWorker.register("./sw.js?v=20260524-3");
