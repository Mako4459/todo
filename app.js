const STORAGE_KEY = "taskai-state-v2";
...
function renderCategoryOptions() {
  ...
  els.category.value = state.categories.includes(els.category.value)
    ? els.category.value
    : state.categories[0];
...
function TaskCard(task) {
  ...
  $(".edit-button", node).addEventListener("click", () => editTask(task.id));
...
function editTask(id) {
  ...
  const category = prompt("カテゴリを編集", task.category);
  if (category === null) return;

  const dueDate = prompt("期限を編集（YYYY-MM-DD / 空欄で期限なし）", task.dueDate || "");
  if (dueDate === null) return;

  const priority = prompt("優先度を編集（high / medium / low）", task.priority);
  if (priority === null) return;

  const memo = prompt("メモを編集", task.memo || "");
  if (memo === null) return;

  const normalizedTitle = title.trim();
  const normalizedCategory = category.trim();
  const normalizedPriority = ["high", "medium", "low"].includes(priority.trim())
    ? priority.trim()
    : task.priority;
  ...
  updateTask(id, (next) => {
    ...
    next.category = normalizedCategory || task.category;
    next.dueDate = dueDate.trim();
    next.priority = normalizedPriority;
    next.memo = memo.trim();
  });
}
...
function exportToObsidian() {
  alert("Obsidian用Markdownを書き出します。");
  ...
  const markdown = [
    ...
    "## 未完了タスク",
    ...active.map((task) => (
      `- [ ] ${task.title} #${task.category} 優先度:${priorityLabel[task.priority]}`
    )),
    "",
    "## 完了タスク",
    ...completed.map((task) => (
      `- [x] ${task.title} #${task.category}`
    )),
    "",
    "## メモ",
    ...state.tasks
      .filter((task) => task.memo)
      .map((task) => (
        `### ${task.title}\n${task.memo}\n`
      )),
  ].join("\n");
  ...
  a.href = url;
  a.download = `taskai-${today}.md`;
  a.click();

  URL.revokeObjectURL(url);

  alert("Obsidian用のMarkdownファイルを書き出しました。");
}
...
function startVoiceInput() {
  ...
  if (els.voiceButton) {
    els.voiceButton.textContent = "聞き取り中...";
    els.voiceButton.disabled = true;
  }

  alert("音声入力を開始しました。話してください。");
  ...
  recognition.onend = () => {
    if (els.voiceButton) {
      els.voiceButton.textContent = "音声入力";
...
document.addEventListener("click", (event) => {
  ...
  if (event.target?.id === "obsidianButton") {
    exportToObsidian();
  }

  if (event.target?.id === "voiceButton") {
    startVoiceInput();
  }
});
